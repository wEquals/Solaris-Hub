...
local title,title2 = ' | beta | v%s | fps %s | ping %s', 'uid: 6 | tg.lol'
if getgenv().swimhub_settings == nil then
    getgenv().swimhub_settings = {
        safemode = true, -- anti AC detection with universal
        nohookdetection = true, -- no index/newindex/namecall detections
        verbosemode = true -- prints current loading state
    }
end
...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('tg.lol | beta | build: dev | uid: 6 | %s fps | %s ms'):format(
        math.floor(FPS),
        math.floor(game:GetService('Stats').Network.ServerStatsItem['Data Ping']:GetValue())
    ));
end);
