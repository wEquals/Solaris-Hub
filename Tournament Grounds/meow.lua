...
local Tabs = {
    ...
    ESPNigga = Window:AddTab('Visuals'),
...
local Aimbottab = Aimbot:AddTab('silent aim')
local WorldTab = Visuals:AddTab('world')
local TabEsp = Tabs.ESPNigga:AddLeftTabbox()
local lpvis = Tabs.ESPNigga:AddRightGroupbox('local player visuals')
...
local luatab = Tabs.Lua:AddRightGroupbox('nigga balls');
(function()
    local draw, objects = {}, {}
    ...
    Misc:AddLabel('Menu bind'):AddKeyPicker('MenuKeybind', { Default = 'Insert', NoUI = true, Text = 'Menu keybind' })
    Library.ToggleKeybind = Misc.MenuKeybind
...
}):AddColorPicker('penisespcolour', {
    Default = Color3.new(1, 1, 1),
    Title = 'penis esp color',
    Transparency = 0,
...
local lpniggachams = false
local lpchamscolour = Color3.fromRGB(255,255,255)

task.spawn(function ()
	while true do
		wait()
		if lpniggachams == true then
			for i,v in pairs(game.Players.LocalPlayer.Character:GetDescendants()) do
				if v:IsA("BasePart") then
					v.Material = "ForceField"
                    v.Color = lpchamscolour
				end
			end
		else
			for i,v in pairs(game.Players.LocalPlayer.Character:GetDescendants()) do
...
lpvis:AddToggle('localchams', {
    Text = 'local player chams',
    Default = false,  
    Callback = function(first)
        lpniggachams = first
    end
}):AddColorPicker('localchamscolormane', {
    Default = Color3.new(1, 1, 1),
    Title = 'local player chams color',
    Transparency = 0,
...
lpvis:AddSlider('backtrackdur', {
    Text = 'backtrack duration',
    Default = 0,
    Min = 0,
    Max = 50,
    Rounding = 1,
...
lpvis:AddSlider('backtracktrans', {
